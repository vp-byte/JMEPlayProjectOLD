...
public class EditorBuilderCenterBorderBar {
	...
	private void initBottom() {
		bottomBox = new HBox();
		bottomBox.getStyleClass().add("borderbar");
		Label labelMargin = new Label();
		labelMargin.setMinWidth(settings.getOptionInteger("guiIconSize"));
		bottomBox.getChildren().add(labelMargin);

		bottomBox.getChildren().addAll(borderItemsBottom);
		borderBarBottom = new BorderBar(settings.getOptionInteger("guiIconSize"), Position.BOTTOM, bottomBox);
		bottomBox.prefHeightProperty().bind(borderBarBottom.heightProperty());
	}
